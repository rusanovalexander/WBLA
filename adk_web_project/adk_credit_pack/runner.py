...
Run from repo root with:  cd <repo_root> && PYTHONPATH=. uv run --project adk_web_project adk web
Or from adk_web_project with:  PYTHONPATH=.. uv run adk web
"""
...
# Setup Google Cloud / Vertex before any agent imports
...
def get_advisor() -> ComplianceAdvisor:
    ...
    if _advisor is None:
        _advisor = ComplianceAdvisor(
            search_guidelines_fn=_search_guidelines,
            governance_context=_governance_context,
            tracer=_tracer,
            search_procedure_fn=_search_procedure,
        )
