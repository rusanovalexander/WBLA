...
def render_rationale(rationale: list):
    """🆕 Render rationale bullets (2–6 bullets) when present; always show when non-empty."""
...
def render_chat():
    ...
    for idx, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"]):
            ...
            # 🆕 Show rationale (bullets) when present — always show expander if non-empty
            if message.get("rationale"):
                render_rationale(message["rationale"])

            # 🆕 Show extended reasoning if exists
            if message.get("reasoning"):
                render_reasoning(message["reasoning"])

            # Show thinking process (use thinking_steps or thinking)
            steps = message.get("thinking_steps") or message.get("thinking")
            if steps:
    ...
    if replay_step is not None and hasattr(st.session_state.orchestrator, "process_replay_from_step"):
        ...
        st.session_state.messages.append({
            "role": "assistant",
            "content": result.get("content") or result["response"],
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            # Display response (content/response — avoid duplicating with streamed chunks)
            st.markdown(result.get("content") or result["response"])

            # 🆕 Show rationale when present (always show expander)
            if result.get("rationale"):
                render_rationale(result["rationale"])

            # 🆕 Show extended reasoning
            if result.get("reasoning"):
                render_reasoning(result["reasoning"])

            # Show thinking in expander (use thinking_steps or thinking)
            steps = result.get("thinking_steps") or result.get("thinking")
            if steps:
                render_thinking_process(
                    steps,
                    "✅ Complete"
            ...
            # Save assistant message (with rationale and thinking_steps for replay)
            st.session_state.messages.append({
                "role": "assistant",
                "content": result.get("content") or result["response"],
                "rationale": result.get("rationale"),
