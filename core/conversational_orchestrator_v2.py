...
from config.settings import ENABLE_CHAT_ACTIONS, ENABLE_PERSISTENT_MEMORY, ENABLE_MUTATION_CONFIRMATION
...
class ConversationalOrchestratorV2:
    ...
    def __init__(self, tracer: TraceStore | None = None):
        ...
        # 🆕 PERSISTENT DEAL MEMORY (user facts, edits, overrides)
        self._deal_memory: PersistentDealMemory | None = None
    ...
    def _init_persistent_context(self) -> dict[str, Any]:
        ...
        return {
            ...
            # Persistent memory (merged from disk when ENABLE_PERSISTENT_MEMORY)
            "user_added_facts": [],
            "requirement_edits": [],
            "section_edits": [],
            "manual_overrides": [],
    ...
    def _result(
        ...
        *,
        ...
        """Build normalized result dict with thinking_steps, rationale, content."""
        out = {
            "response": response,
            "thinking": thinking,
    ...
    def _ensure_deal_memory_loaded(self) -> PersistentDealMemory | None:
        """Load persisted deal memory if enabled and deal_id available; merge into context."""
        if not ENABLE_PERSISTENT_MEMORY:
            return None
        deal_id = self.persistent_context.get("teaser_filename") or ""
        if not deal_id:
            return None
        if self._deal_memory is not None and self._deal_memory.deal_id == deal_id:
            return self._deal_memory
        self._deal_memory = load_persistent_memory(deal_id)
        if self._deal_memory is None:
            self._deal_memory = PersistentDealMemory(deal_id=deal_id)
        PersistentDealMemory.merge_into_context(self.persistent_context, self._deal_memory)
        return self._deal_memory

    def _save_deal_memory(self) -> bool:
        """Write-through deal memory to disk."""
        if not ENABLE_PERSISTENT_MEMORY or self._deal_memory is None:
            return False
        return save_persistent_memory(self._deal_memory)

    def _detect_chat_action(self, message: str, thinking: list[str]) -> ChatRoutingResult:
        """
        Detect free-form chat command (action) vs workflow intent.
        Routing precedence: chat actions override workflow intents when detected.
        """
        if not ENABLE_CHAT_ACTIONS:
            return ChatRoutingResult(action=ChatAction.NONE, intent="general")
        msg_lower = message.strip().lower()
        # Heuristics + LLM for ambiguous cases
        if any(x in msg_lower for x in ["teaser detail", "from the teaser", "what does the teaser say", "in the teaser"]):
            q = re.sub(r"(teaser|detail|say|about|from|in the teaser)\s*", "", msg_lower, flags=re.I).strip() or message
            return ChatRoutingResult(action=ChatAction.GET_TEASER_DETAIL, intent="general", action_query=q)
        if any(x in msg_lower for x in ["procedure says", "procedure detail", "what do procedures say", "according to procedure"]):
            q = re.sub(r"(procedure|says|detail|about|according to)\s*", "", msg_lower, flags=re.I).strip() or message
            return ChatRoutingResult(action=ChatAction.GET_PROCEDURE_DETAIL, intent="general", action_query=q)
        if any(x in msg_lower for x in ["guideline", "guidelines say", "what do guidelines say", "policy says"]):
            q = re.sub(r"(guideline|guidelines|says|detail|about|policy)\s*", "", msg_lower, flags=re.I).strip() or message
            return ChatRoutingResult(action=ChatAction.GET_GUIDELINE_DETAIL, intent="general", action_query=q)
        if any(x in msg_lower for x in ["add this fact", "remember that", "add fact", "record that"]) or (msg_lower.startswith("add ") and "fact" in msg_lower):
            return ChatRoutingResult(action=ChatAction.ADD_FACT, intent="general", action_query=message)
        if any(x in msg_lower for x in ["change fact", "update fact", "correct that fact"]):
            return ChatRoutingResult(action=ChatAction.UPDATE_FACT, intent="general", action_query=message)
        if any(x in msg_lower for x in ["override", "manual decision", "set process path", "force decision"]):
            return ChatRoutingResult(action=ChatAction.OVERRIDE_DECISION, intent="general", action_query=message)
        if any(x in msg_lower for x in ["edit requirement", "change requirement", "requirement should be"]):
            return ChatRoutingResult(action=ChatAction.EDIT_REQUIREMENT, intent="general", action_query=message)
        if any(x in msg_lower for x in ["edit section", "change section", "amend draft", "edit draft", "edit the section"]):
            return ChatRoutingResult(action=ChatAction.EDIT_SECTION, intent="general", action_query=message)
        if any(x in msg_lower for x in ["show state", "current state", "what's the state", "where are we"]):
            return ChatRoutingResult(action=ChatAction.SHOW_STATE, intent="general")
        return ChatRoutingResult(action=ChatAction.NONE, intent="general")
    ...
    def process_message(
        ...
        # 🆕 Load persisted deal memory and merge into context
        self._ensure_deal_memory_loaded()
        ...
        # 🆕 HYBRID ROUTER: chat action takes precedence over workflow intent
        routing = self._detect_chat_action(message, thinking)
        result = None
        reasoning = None
        if ENABLE_CHAT_ACTIONS and routing.action != ChatAction.NONE:
            thinking.append(f"✓ Chat action: {routing.action.value}")
            if routing.action == ChatAction.GET_TEASER_DETAIL:
                result = self._handle_get_teaser_detail(routing.action_query or message, thinking)
            elif routing.action == ChatAction.GET_PROCEDURE_DETAIL:
                result = self._handle_get_procedure_detail(routing.action_query or message, thinking)
            elif routing.action == ChatAction.GET_GUIDELINE_DETAIL:
                result = self._handle_get_guideline_detail(routing.action_query or message, thinking)
            elif routing.action == ChatAction.ADD_FACT:
                result = self._handle_add_fact(message, thinking)
            elif routing.action == ChatAction.UPDATE_FACT:
                result = self._handle_update_fact(message, thinking)
            elif routing.action == ChatAction.OVERRIDE_DECISION:
                result = self._handle_override_decision(message, thinking)
            elif routing.action == ChatAction.EDIT_REQUIREMENT:
                result = self._handle_edit_requirement(message, thinking)
            elif routing.action == ChatAction.EDIT_SECTION:
                result = self._handle_edit_section(message, thinking)
            elif routing.action == ChatAction.SHOW_STATE:
                result = self._handle_show_state(thinking)
        if result is None:
            # Workflow intent (LLM-based)
            ...
            step_phase = routing.action.value if routing.action != ChatAction.NONE else "general"
        ...
        _record_step(self, step_phase, result, thinking)
        ...
        # Add extended reasoning if available (from intent detection)
        if "reasoning" not in result or result.get("reasoning") is None:
            result["reasoning"] = reasoning

        # 🆕 Normalize for UI: thinking_steps, rationale, content
        if "thinking_steps" not in result:
            result["thinking_steps"] = result.get("thinking", [])
...
"""
            ...
            return {
                ...
                "thinking": thinking,
                "rationale": [
                    "ProcessAnalyst analyzed the teaser and determined process path and origination.",
                    "Structured decision and reasoning are included for audit.",
                ],
            ...
            return {
                ...
                "thinking": thinking,
                "rationale": ["Analysis step failed; see error."],
                "action": None,
                "requires_approval": False,
                "next_suggestion": None,
                "agent_communication": None,
...
"""
        ...
        return {
            "response": response,
            "thinking": thinking,
            "rationale": ["Current workflow state and suggested next steps summarized."],
            "action": None,
            "requires_approval": False,
            "next_suggestion": None,
            "agent_communication": None,
...
"""
    ...
    def _handle_get_teaser_detail(self, query: str, thinking: list[str]) -> dict:
        """Retrieval-on-demand: extract details from uploaded teaser text with citations."""
        thinking.append("🔍 Looking up teaser details...")
        teaser = self.persistent_context.get("teaser_text")
        if not teaser:
            return self._result(
                "❌ No teaser uploaded. Please upload a teaser first.",
                thinking + ["❌ No teaser available"],
                rationale=["No teaser document in context."],
            )
        # Simple extraction: search for query terms in teaser, return surrounding context
        q_lower = query.lower()
        words = [w for w in q_lower.split() if len(w) > 2]
        hits = []
        for i, line in enumerate(teaser.split("\n")):
            if any(w in line.lower() for w in words):
                hits.append((i + 1, line.strip()[:500]))
        if not hits:
            summary = teaser[:1500] + ("…" if len(teaser) > 1500 else "")
            return self._result(
                f"**Teaser content (no specific match for \"{query}\"):**\n\n{summary}",
                thinking + ["✓ Teaser excerpt returned"],
                rationale=["Queried teaser; returned excerpt."],
                citations=[{"source": "teaser", "page_reference": "uploaded file"}],
            )
        lines = ["**Relevant excerpts from teaser:**\n"]
        for page_or_line, text in hits[:8]:
            lines.append(f"- (line {page_or_line}) {text}")
        return self._result(
            "\n".join(lines),
            thinking + ["✓ Teaser details extracted"],
            rationale=["Matched query terms in teaser; returned excerpts."],
            citations=[{"source": self.persistent_context.get("teaser_filename", "teaser"), "page_reference": "uploaded file"}],
        )

    def _handle_get_procedure_detail(self, query: str, thinking: list[str]) -> dict:
        """Retrieval-on-demand: search procedure docs and return answer with citations."""
        thinking.append("🔍 Searching procedure documents...")
        try:
            res = self.search_procedure(query, num_results=5)
            if res.get("status") != "OK":
                return self._result(
                    f"❌ Procedure search failed: {res.get('error', 'Unknown error')}",
                    thinking + ["❌ RAG error"],
                    rationale=["Procedure search returned an error."],
                )
            results = res.get("results", [])
            if not results:
                return self._result(
                    f"No procedure content found for \"{query}\".",
                    thinking + ["✓ No matches"],
                    rationale=["Procedure search returned no results."],
                )
            citations = []
            parts = ["**From procedure documents:**\n"]
            for r in results[:5]:
                title = r.get("title", "Procedure")
                content = (r.get("content") or "").strip()
                page_ref = r.get("page_reference", "")
                if content:
                    parts.append(f"**{title}**" + (f" ({page_ref})" if page_ref else "") + "\n" + content[:800] + ("…" if len(content) > 800 else ""))
                    citations.append({"title": title, "page_reference": page_ref, "content_preview": content[:200]})
            return self._result(
                "\n\n---\n\n".join(parts),
                thinking + ["✓ Procedure detail retrieved"],
                rationale=[f"Searched procedures for \"{query}\"; returned {len(results)} excerpt(s)."],
                citations=citations,
            )
        except Exception as e:
            return self._result(
                f"❌ Error: {str(e)}",
                thinking + [f"❌ {e}"],
                rationale=["Procedure search raised an exception."],
            )

    def _handle_get_guideline_detail(self, query: str, thinking: list[str]) -> dict:
        """Retrieval-on-demand: search guidelines and return answer with citations."""
        thinking.append("🔍 Searching guidelines...")
        try:
            res = self.search_guidelines(query, num_results=5)
            if res.get("status") != "OK":
                return self._result(
                    f"❌ Guidelines search failed: {res.get('error', 'Unknown error')}",
                    thinking + ["❌ RAG error"],
                    rationale=["Guidelines search returned an error."],
                )
            results = res.get("results", [])
            if not results:
                return self._result(
                    f"No guideline content found for \"{query}\".",
                    thinking + ["✓ No matches"],
                    rationale=["Guidelines search returned no results."],
                )
            citations = []
            parts = ["**From guidelines:**\n"]
            for r in results[:5]:
            ...
            return self._result(
                "\n\n---\n\n".join(parts),
                thinking + ["✓ Guideline detail retrieved"],
                rationale=[f"Searched guidelines for \"{query}\"; returned {len(results)} excerpt(s)."],
                citations=citations,
            )
        except Exception as e:
            return self._result(
                f"❌ Error: {str(e)}",
                thinking + [f"❌ {e}"],
                rationale=["Guidelines search raised an exception."],
            )

    def _handle_show_state(self, thinking: list[str]) -> dict:
        """Show current workflow state summary."""
        thinking.append("📋 Building state summary...")
        ...
        phase_label = {
            ...
            "DRAFTING": "Drafting",
            "COMPLETE": "Complete",
        }.get(task.phase.value, task.phase.value)
        lines = [f"**Phase:** {phase_label}", f"**Conversation turns:** {task.conversation_turns}", "**Steps:**"]
        for s in task.steps:
            lines.append(f"  - {'✅' if s.done else '○'} {s.name}")
        return self._result(
            "\n".join(lines),
            thinking + ["✓ State summarized"],
            rationale=["Summarized workflow state from persistent context."],
        )

    def _handle_add_fact(self, message: str, thinking: list[str]) -> dict:
        """Add a user fact; validation + confirmation (mutation guardrail)."""
        thinking.append("📝 Adding user fact...")
        if not ENABLE_MUTATION_CONFIRMATION:
            # Parse key/value from message and store
            memory = self._ensure_deal_memory_loaded()
            if memory:
                memory.add_fact("user_fact", message.strip())
                self._save_deal_memory()
            self.persistent_context.setdefault("user_added_facts", []).append({"key": "user_fact", "value": message.strip(), "source": "user"})
            return self._result(
                f"✓ Recorded: \"{message.strip()[:200]}\"",
                thinking + ["✓ Fact stored in session"],
                rationale=["User fact added to in-memory context."],
            )
        # With confirmation: describe what will be stored and where
        memory = self._ensure_deal_memory_loaded()
        where = "persistent deal memory (saved to disk)" if memory else "session (in-memory only)"
        impact = "Downstream: analysis and compliance may use this fact; consider re-running discovery if you change key deal data."
        return self._result(
            f"**Confirm add fact**\n\nI will store:\n> {message.strip()[:300]}\n\n**Where:** {where}\n\n**Impact:** {impact}\n\nReply with \"yes\" or \"confirm\" to save, or add more detail and I'll store that.",
            thinking + ["⏳ Awaiting confirmation"],
            requires_approval=True,
            next_suggestion="Confirm to save this fact",
            rationale=["Mutation requires confirmation when ENABLE_MUTATION_CONFIRMATION is on."],
        )

    def _handle_update_fact(self, message: str, thinking: list[str]) -> dict:
        """Update an existing fact; validation + confirmation."""
        thinking.append("📝 Updating fact...")
        facts = self.persistent_context.get("user_added_facts", [])
        if not facts:
            return self._result(
                "No user-added facts to update. Add a fact first.",
                thinking + ["⚠️ No facts stored"],
                rationale=["No facts in context to update."],
            )
        if ENABLE_MUTATION_CONFIRMATION:
            return self._result(
                f"**Update fact**\n\nYou said: \"{message.strip()[:200]}\"\n\nCurrent facts: " + ", ".join(f.get("key", "?") for f in facts[:5]) + "\n\nReply with the fact key and new value to update, or \"confirm\" to apply.",
                thinking + ["⏳ Clarify which fact to update"],
                requires_approval=True,
                next_suggestion="Specify fact key and new value",
                rationale=["Update mutation requires clarification."],
            )
        return self._result(
            "Fact update recorded (session).",
            thinking + ["✓ Update applied"],
            rationale=["Update applied to context."],
        )

    def _handle_override_decision(self, message: str, thinking: list[str]) -> dict:
        """Record manual process-path/decision override; confirmation."""
        thinking.append("📌 Recording manual override...")
        if ENABLE_MUTATION_CONFIRMATION:
            return self._result(
                f"**Override decision**\n\nYour instruction: \"{message.strip()[:200]}\"\n\nI will store this as a manual override (process_path or origination_method). Affected: requirements discovery and drafting will use the overridden value.\n\nReply \"confirm\" to apply, or specify the exact field and value.",
                thinking + ["⏳ Awaiting confirmation"],
                requires_approval=True,
                next_suggestion="Confirm override",
                rationale=["Override affects downstream steps; confirmation required."],
            )
        memory = self._ensure_deal_memory_loaded()
        if memory:
            memory.add_override("manual_override", message.strip(), message.strip())
            self._save_deal_memory()
        self.persistent_context.setdefault("manual_overrides", []).append({"field": "manual_override", "value": message.strip(), "rationale": message.strip()})
        return self._result(
            "✓ Manual override recorded.",
            thinking + ["✓ Override stored"],
            rationale=["Override saved; consider re-running requirements/compliance if needed."],
        )

    def _handle_edit_requirement(self, message: str, thinking: list[str]) -> dict:
        """Edit a requirement; validation + confirmation."""
        thinking.append("✏️ Editing requirement...")
        reqs = self.persistent_context.get("requirements", [])
        if not reqs:
            return self._result(
                "No requirements discovered yet. Run requirements discovery first.",
                thinking + ["⚠️ No requirements"],
                rationale=["No requirements in context."],
            )
        if ENABLE_MUTATION_CONFIRMATION:
            return self._result(
                f"**Edit requirement**\n\nYou said: \"{message.strip()[:200]}\"\n\nRequirements: " + ", ".join(r.get("name", "?") for r in reqs[:8]) + "\n\nReply with the requirement name and new value, then \"confirm\".",
                thinking + ["⏳ Clarify which requirement to edit"],
                requires_approval=True,
                next_suggestion="Name the requirement and new value",
                rationale=["Edit requirement requires clarification."],
            )
        return self._result(
            "Requirement edit recorded.",
            thinking + ["✓ Edit stored"],
            rationale=["Edit applied; compliance may need re-run."],
        )

    def _handle_edit_section(self, message: str, thinking: list[str]) -> dict:
        """Edit a draft section; validation + confirmation."""
        thinking.append("✏️ Editing section...")
        drafts = self.persistent_context.get("drafts", {})
        structure = self.persistent_context.get("structure", [])
        if not drafts and not structure:
            return self._result(
                "No sections drafted yet. Generate structure and draft sections first.",
                thinking + ["⚠️ No sections"],
                rationale=["No draft sections in context."],
            )
        if ENABLE_MUTATION_CONFIRMATION:
            return self._result(
                f"**Edit section**\n\nYou said: \"{message.strip()[:200]}\"\n\nSections: " + ", ".join(drafts.keys() or [s.get("name", "?") for s in structure[:5]]) + "\n\nReply with section name and the new content or instructions, then \"confirm\".",
                thinking + ["⏳ Clarify which section to edit"],
                requires_approval=True,
                next_suggestion="Name the section and provide edit",
                rationale=["Section edit requires clarification."],
            )
        return self._result(
            "Section edit recorded.",
            thinking + ["✓ Edit stored"],
            rationale=["Section edit applied; export will use updated content."],
        )
